...
// Home is the landing tab: most sessions are 'play the thing I already set
// up', not 'manage my library'.
let tab: TabId = 'home';
let search = '';
let filter = 'all';
...
function renderOrder(s: AppState, view: HTMLElement): void {
  ...
  // Design 1c: the order on the left, the consequence of it on the right.
  const split = el('div', 'order-split');
  const left = el('div', 'order-stack');
  left.appendChild(stack);
  split.appendChild(left);

  const right = el('div', 'order-diff');
  renderSwapDiff(s, right, orderPlan);
  ...
  // The plan is a main-process round trip, so it is fetched once per profile
  // state rather than on every keystroke, and painted in when it lands.
  void refreshOrderPlan(profile.id, right);
}

/**
 * The swap plan shown beside the load order.
 *
 * Cached against the profile it was computed for so that dragging a row does
 * not fire a fresh plan request per frame; `planKey` changes whenever anything
 * that could alter the outcome does.
 */
let orderPlan: SwapPlan | null = null;
let orderPlanKey = '';

async function refreshOrderPlan(profileId: string, host: HTMLElement): Promise<void> {
  const s = state;
  if (!s) return;
  const profile = s.profiles.find((p) => p.id === profileId);
  if (!profile) return;

  // Order and enabled set both change the answer; so does the excluded-file
  // set, which is why it is part of the key rather than assumed constant.
  const key = [
    profileId,
    profile.order.join(','),
  ...
  if (key === orderPlanKey && orderPlan) return;

  try {
    const plan = await api.planSwap(profileId);
    orderPlanKey = key;
    orderPlan = plan;
    // Only paint if this host is still on screen: the user may have changed
    // tabs while the round trip was in flight.
    if (host.isConnected) renderSwapDiff(s, host, plan);
  } catch {
    // A plan that cannot be built is not worth a toast - the Apply button
    // reports the same problem properly when it matters.
  }
}

// --- rendering: home (design 2a) --------------------------------------------

/**
 * "Which setup do you want to play?"
 *
 * Design 2a, which had never been built. Every other tab is for *managing*
 * mods; this one exists for the far more common case of having already set
 * things up and just wanting to play. One card per profile, one obvious
 * button, and the numbers that decide between them stated in plain words.
 */
function renderHome(s: AppState, view: HTMLElement): void {
  ...
  const head = el('div');
  head.appendChild(el('h1', 'home-ask', 'Which setup do you want to play?'));
  head.appendChild(
    el(
      'p',
      'home-lede',
      'Pick a setup and press Play. Swapmeet moves the mods into place for you, keeps your save files, and shelves anything it displaces instead of deleting it.',
    ),
  );
  ...
  for (const profile of s.profiles) {
    grid.appendChild(setupCard(s, profile));
  }

  const add = el('button', 'setup-card');
  add.appendChild(el('div', 'setup-name', 'Make a new setup'));
  add.appendChild(
    el('div', 'setup-blurb', 'Start empty, or copy one you already have.'),
  );
...
function setupCard(s: AppState, profile: Profile): HTMLElement {
  const isLive = s.deployed?.profileId === profile.id;
  const card = el('div', `setup-card${isLive ? ' is-live' : ''}`);

  const head = el('div', 'setup-head');
  head.appendChild(el('div', 'setup-name', profile.name));
  if (isLive) {
    head.appendChild(el('div', 'setup-tag is-live', 'IN USE'));
  } else if (profile.vanillaLock) {
    head.appendChild(el('div', 'setup-tag is-safe', 'SAFE'));
  }
  ...
  card.appendChild(
    el(
      ...
      'setup-blurb',
      profile.vanillaLock
        ? 'No mods at all. Use this before going online, so nothing modded is in the game folder.'
        : describeProfile(s, profile),
    ),
  );

  const facts = el('div', 'setup-facts');
  const enabled = profile.vanillaLock ? 0 : profile.enabled.length;
  const bytes = s.mods
    .filter((m) => !profile.vanillaLock && profile.enabled.includes(m.id))
    .reduce((sum, m) => sum + m.size, 0);
  facts.appendChild(fact('Mods switched on', enabled === 0 ? 'None' : String(enabled)));
  facts.appendChild(fact('Size on disk', enabled === 0 ? '—' : formatBytes(bytes)));
  facts.appendChild(
    fact('Last played', profile.lastLaunchedAt ? formatDate(profile.lastLaunchedAt) : 'Never'),
  );
  card.appendChild(facts);

  card.appendChild(el('div', 'setup-spacer'));

  const play = el(
    'button',
    isLive ? 'solid-btn' : 'outline-btn',
    isLive ? 'Play now' : 'Switch and play',
  );
...
/**
 * A one-line description of a profile, in the words someone would use out
 * loud rather than the words the file system would use.
 */
function describeProfile(s: AppState, profile: Profile): string {
  const mods = s.mods.filter((m) => profile.enabled.includes(m.id));
  if (mods.length === 0) return 'Nothing switched on yet. Open it to add some mods.';
  const named = mods
    .filter((m) => !m.core)
    .slice(0, 3)
    .map((m) => m.name);
  if (named.length === 0) return 'Just the pieces needed to run mods, and nothing else yet.';
  const rest = mods.filter((m) => !m.core).length - named.length;
  return rest > 0 ? `${named.join(', ')} and ${rest} more.` : `${named.join(', ')}.`;
}

/**
 * Make a profile current and launch straight into it.
 *
 * Deliberately routed through the same apply path as the Apply button rather
 * than a shortcut of its own: the preview, the blockers and the online warning
 * all still have to happen. "Play" is a faster route to the same decision, not
 * a way around it.
 */
...
/**
 * The right-hand half of the load-order view: what is about to move.
 *
 * Design 1c. The load order on its own tells you the rules; this tells you the
 * consequence, before you commit to it. It is the same plan the Apply button
 * runs, shown while you are still editing rather than in a dialog after you
 * have already decided - which is the whole difference between a preview and a
 * confirmation prompt.
 */
function renderSwapDiff(s: AppState, host: HTMLElement, plan: SwapPlan | null): void {
  clear(host);
  const profile = s.profiles.find((p) => p.id === s.activeProfileId);

  host.appendChild(
    el('div', 'swap-eyebrow', `Swapping to · ${profile?.name ?? 'this setup'}`),
  );

  if (!plan) {
    host.appendChild(el('div', 'swap-headline', 'Working out what moves…'));
    return;
  }
  ...
  host.appendChild(
    el(
      ...
      'swap-headline',
      moving === 0
        ? 'Nothing to move — this setup is already installed'
        : `${moving} file${moving === 1 ? '' : 's'} move, nothing is deleted`,
    ),
  );
  ...
  const parts = [
    `${plan.filesIn} in`,
    `${plan.filesOut} out`,
    `${plan.filesKept} left alone`,
  ];
  host.appendChild(
    el(
      'div',
      'swap-sub',
      `${parts.join(' · ')}. Anything already in the game folder that a mod replaces is moved to the shelf, so it can be put back.`,
    ),
  );

  for (const blocker of plan.blockers) {
    host.appendChild(banner('warn', 'This cannot be applied yet', blocker));
  }
  for (const warning of plan.warnings) {
    host.appendChild(banner('info', 'Worth knowing', warning));
  }

  if (plan.entries.length > 0) {
    const rows = el('div', 'swap-rows');
    // Capped: entries are per mod, so this is generous, but a several-hundred
    // row list is not a preview - it is a wall to scroll past.
    const shown = plan.entries.slice(0, 120);
    for (const entry of shown) {
      const row = el('div', `swap-row is-${entry.kind}`);
      row.appendChild(el('div', 'swap-sign', entry.kind.toUpperCase()));
      row.appendChild(
        el(
          'div',
          'swap-name',
          `${entry.name}  ·  ${entry.fileCount} file${entry.fileCount === 1 ? '' : 's'}`,
        ),
      );
      const path = el('div', 'swap-path', entry.path);
      path.title = entry.path;
      row.appendChild(path);
      rows.appendChild(row);
    }
    host.appendChild(rows);
    if (plan.entries.length > shown.length) {
      host.appendChild(
        el('div', 'swap-sub', `…and ${plan.entries.length - shown.length} more mods.`),
      );
    }
  }

  const cards = el('div', 'swap-cards');

  // Roadmap item 9: the disk cost, stated before it is spent rather than
  // discovered afterwards on a full drive.
  const disk = el('div', 'swap-card');
  disk.appendChild(el('div', 'swap-eyebrow', 'Disk space'));
  disk.appendChild(
    el(
      'div',
      'swap-card-body',
      plan.bytesToWrite > 0
        ? `About ${formatBytes(plan.bytesToWrite)} gets written. Where the library and the game sit on the same drive these are hard links, which cost almost nothing; across drives they have to be real copies.`
        : 'Nothing new needs writing — every file this setup wants is already in place.',
    ),
  );
  cards.appendChild(disk);

  const savesCard = el('div', 'swap-card');
  savesCard.appendChild(el('div', 'swap-eyebrow', 'Save files'));
  savesCard.appendChild(
    el(
      'div',
      'swap-card-body',
      s.settings.backupSavesOnSwap
        ? `A snapshot is taken before every switch. ${saves.length} kept so far.`
        : 'Snapshots are switched off, so your saves are not copied before a switch.',
    ),
  );
  cards.appendChild(savesCard);

  const gfx = el('div', 'swap-card');
  gfx.appendChild(el('div', 'swap-eyebrow', 'Graphics settings'));
  gfx.appendChild(
    el(
      'div',
      'swap-card-body',
      s.settings.graphicsPerProfile
        ? 'This setup carries its own graphics and launch settings, applied on the switch.'
        : 'All setups share one graphics file, so a change made in one shows up in the others.',
    ),
  );
  cards.appendChild(gfx);

  host.appendChild(cards);
}

/** A small titled notice, used for both the banners and inline plan warnings. */
function banner(
  tone: 'warn' | 'info',
  title: string,
  body: string,
  ...
  main.appendChild(el('div', 'banner-title', title));
  main.appendChild(el('div', 'banner-body', body));
  if (code) main.appendChild(el('div', 'banner-code', code));
  if (actions.length > 0) {
    const row = el('div', 'banner-actions');
    for (const action of actions) {
      const btn = el(
        'button',
        `small-btn${action.primary ? ' is-primary' : ''}`,
        action.label,
      );
...
/**
 * The alerts that sit above every tab.
 *
 * Above rather than inside the view on purpose: a game update or a missing
 * dlclist entry is exactly the thing someone goes hunting through the tabs to
 * explain, so it has to still be there when they reach the tab they suspect.
 */
function renderBanners(s: AppState): void {
  const host = byId('banners');
  clear(host);

  if (s.buildAlert) {
    const alert = s.buildAlert;
    const affected = alert.affected.length;
    const names = alert.affected.slice(0, 4).map((m) => m.name).join(', ');
    const rest = affected - Math.min(affected, 4);

    // Three genuinely different situations, and saying the wrong one is worse
    // than saying nothing: "unknown" is common, because a copy adopted out of
    // a game folder is just ScriptHookV.dll with no version anywhere in it.
    const hookLine =
    ...
    const body =
      affected === 0
        ? `${alert.gameName} went from ${alert.previous} to ${alert.current}. Nothing you have switched on runs through Script Hook V, so this probably changes nothing for you.${hookLine}`
        : `${alert.gameName} went from ${alert.previous} to ${alert.current}. Every game update moves the internals Script Hook V hooks into, so script mods stop loading until it catches up. This is not something you broke.${hookLine} Affected: ${names}${rest > 0 ? ` and ${rest} more` : ''}.`;

    host.appendChild(
      banner(
        affected > 0 ? 'warn' : 'info',
  ...
  const dlc = s.dlc;
  if (dlc && dlc.gaps.length > 0) {
    const first = dlc.gaps[0]!;
    const more = dlc.gaps.length - 1;
    host.appendChild(
      banner(
        'warn',
        dlc.gaps.length === 1
        ...
        }. Without the entry the pack installs perfectly and the game ignores it completely. Add it with OpenIV or CodeWalker:`,
        more > 0
          ? [
              {
                  ...
                  openModal({
                    ...
                    build: (body) => {
                      for (const gap of dlc.gaps) {
                        const row = el('div', 'banner-code', gap.line);
